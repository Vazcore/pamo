...
namespace lbvh
{
    ...
    namespace detail
    {
        ...
        template <typename UInt>
        __device__ inline unsigned int find_split(UInt const *node_code, const unsigned int num_leaves,
                              ...
                              UInt const *node_code, const unsigned int num_objects)
{
    ...
    struct default_morton_code_calculator
    {
        ...
        __device__ __host__ inline unsigned int operator()(const Object &, const aabb<Real> &box) noexcept
        {
            auto p = centroid(box);
            p.x -= whole.lower.x;
            p.y -= whole.lower.y;
    ...
    public:
        ...
        void construct()
        {
            ...
            // higher 32bit : morton code , lower 32bit : index
            ...
            thrust::transform(morton.begin(), morton.end(), indices.begin(),
                              ...
                              [] __host__ __device__(const unsigned int m, const unsigned int idx)
                              {
                                  ...
                                  unsigned long long int m64 = static_cast<unsigned long long int>(m);
                                  m64 <<= 32;
                                  m64 |= idx;
                                  return m64;
                              });
