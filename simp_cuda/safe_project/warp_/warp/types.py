...
class array(Array):
    ...
    def _init_from_data(self, data, dtype, shape, device, copy, pinned):
        ...
        if dtype == Any:
            # infer dtype from data (np.asarray allows copy when needed; NumPy 2.x compat)
            try:
            ...
            try:
                # np.asarray allows copy when needed (e.g. dtype conversion); NumPy 2.x compat
                arr = np.atleast_1d(np.asarray(data, dtype=npdtype))
            except Exception as e:
    ...
    def numpy(self):
        if self.ptr:
            ...
            # np.asarray allows copy when needed; NumPy 2.x compat
            return np.asarray(a)
        else:
